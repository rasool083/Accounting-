...
var DB={
  people:[],products:[],prices:[],sales:[],receipts:[],payments:[],checks:[],purchases:[],saleReturns:[],purchaseReturns:[],accounts:[],transfers:[],expenses:[],incomes:[],adjustments:[],audit:[],operations:[],effects:[],receiptAllocations:[],settings:null,
  load:function(){
    var state=window.Repository?Repository.loadAll():null;
    this.people=state?state.people:Store.get("people",[]);this.products=state?state.products:Store.get("products",[]);this.prices=state?state.prices:Store.get("prices",[]);this.sales=state?state.sales:Store.get("sales",[]);this.receipts=state?state.receipts:Store.get("receipts",[]);this.payments=state?state.payments:Store.get("payments",[]);this.checks=state?state.checks:Store.get("checks",[]);this.purchases=state?state.purchases:Store.get("purchases",[]);this.saleReturns=state?state.saleReturns:Store.get("saleReturns",[]);this.purchaseReturns=state?state.purchaseReturns:Store.get("purchaseReturns",[]);this.accounts=state?state.accounts:Store.get("accounts",[]);this.transfers=state?state.transfers:Store.get("transfers",[]);this.expenses=state?state.expenses:Store.get("expenses",[]);this.incomes=state?state.incomes:Store.get("incomes",[]);this.adjustments=state?state.adjustments:Store.get("adjustments",[]);this.audit=state?state.audit.slice(-500):Store.get("audit",[]).slice(-500);this.operations=state?state.operations:Store.get("operations",[]);this.effects=state?state.effects:Store.get("effects",[]);this.receiptAllocations=state?state.receiptAllocations:Store.get("receiptAllocations",[]);this.settings=(state&&state.settings)||Store.get("settings",null)||this.defaultSettings()
  },
  save:function(what){if(Array.isArray(this[what]))return window.Repository?Repository.save(what,this[what]):Store.set(what,this[what]);if(what==="settings")return Store.set("settings",this.settings)},
...
function fifo(customerId,calcDate){return window.Finance?Finance.fifo({customerId:customerId,calcDate:calcDate,settings:DB.settings,sales:DB.sales,receipts:DB.receipts}):{alloc:[],credit:0,totalInt:0,totalRem:0,totalSV:0,totalOrig:0,receiptAllocations:[]}}
