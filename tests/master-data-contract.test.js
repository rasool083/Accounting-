...
test('people are multi-role and keep payment package default',()=>{
  ...
  const person=DB.addPerson({name:'میثم',company:'فروشگاه میثم',phone:'0912',address:'تهران',roles:['customer','supplier'],paymentPackageId:'St1',notes:'تست'});
  assert.deepEqual(Array.from(person.roles).sort(),['customer','supplier']);
...
test('products support warehouse/location and predefined sales units',()=>{
  const DB=loadDB();
  const p=DB.addProduct({code:'Z',name:'کالای Z',baseUnit:'عدد',packageUnit:'کارتن',unitsPerPackage:36,pricingUnit:'عدد',warehouseId:'FG'});
  assert.equal(p.warehouseId,'FG');
  assert.deepEqual(Array.from(p.salesUnits),['عدد','کارتن']);
...
test('editing a person preserves identity and updates fields',()=>{
  ...
  assert.deepEqual(Array.from(edited.roles).sort(),['customer','partner']);
