...
(function(root, factory){
  ...
  root.DateEngine=api.DateEngine;
  root.Finance=api.Finance;
  ...
  function j2g(jy,jm,jd){
    ...
    var md=[0,31,((gy%4===0&&gy%100!==0)||gy%400===0)?29:28,31,30,31,30,31,31,30,31,30,31];
    var gm=1;while(gm<=12&&d>=md[gm]){d-=md[gm];gm++}
    return [gy,gm,d+1]
  }
  function parseJ(s){
  ...
  function fifo(input){
    input=input||{};var customerId=input.customerId,calc=parseJ(input.calcDate);
    if(!calc)throw new Error("calcDate must be YYYY/MM/DD");
    var settings=input.settings||{dayBasis:30,tiers:[]};
    var sales=(input.sales||[]).filter(function(x){return x.customerId===customerId&&parseJ(x.jDate)}).slice().sort(function(a,b){return String(a.jDate).localeCompare(String(b.jDate))||String(a.id).localeCompare(String(b.id))});
    var receipts=(input.receipts||[]).filter(function(x){return x.customerId===customerId&&x.status!=="باطل"&&x.status!=="برگشتی"&&parseJ(x.jDate)&&String(x.jDate)<=String(input.calcDate)}).slice().sort(function(a,b){return String(a.jDate).localeCompare(String(b.jDate))||String(a.id).localeCompare(String(b.id))});
    var alloc=sales.map(function(s){return {sale:s,orig:Number(s.amount)||0,remaining:Number(s.amount)||0,days:0,mult:1,settlement:0,interest:0}});
    var credit=0;
    ...
    alloc.forEach(function(a){
      ...
      var d=diffJ(parseJ(a.sale.jDate),calc);a.days=d;
      a.mult=getMultiplier(d,settings.tiers,settings.dayBasis);
  ...
  function customerSummary(input){
    input=input||{};var f=fifo(input);
    var sales=(input.sales||[]).filter(function(x){return x.customerId===input.customerId&&String(x.jDate)<=String(input.calcDate)});
    var receipts=(input.receipts||[]).filter(function(x){return x.customerId===input.customerId&&x.status!=="باطل"&&x.status!=="برگشتی"&&String(x.jDate)<=String(input.calcDate)});
